import { t as e } from "./rolldown-runtime.BB3S47xo.mjs";
import { n as t, t as n } from "./shared-lib.CQP05-fP.mjs";
function r(e, n) {
  return {
    breakpoints: [
      { hash: `hp57vz`, mediaQuery: `(min-width: 1200px)` },
      { hash: `emqh7v`, mediaQuery: `(min-width: 810px) and (max-width: 1199.98px)` },
      { hash: `mk2wu9`, mediaQuery: `(max-width: 809.98px)` },
    ],
    description: t(e, n).description,
    elements: { NiOM68_OU: `case-studies` },
    robots: `max-image-preview:large`,
    serializationId: `framer-9OjU1`,
    title: t(e, n).title || `Case Studies`,
    viewport: `width=device-width`,
  };
}
var i,
  a,
  o = e(() => {
    (n(),
      (i = 1),
      (a = {
        exports: {
          default: { type: `function`, annotations: { framerContractVersion: `1` } },
          metadataVersion: { type: `variable`, annotations: { framerContractVersion: `1` } },
          __FramerMetadata__: { type: `variable` },
        },
      }));
  });
export { i, o as n, r, a as t };
//# sourceMappingURL=IcLi543Pr.h3vyxv5V.mjs.map
