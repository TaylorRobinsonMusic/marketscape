...
function xe(h) {
  try {
    let i = localStorage.getItem(Le(h));
    return i && i !== w ? i : null;
  } catch {
    return null;
  }
    ...
    readFirstPage() {
      ...
      if (
        ...
        let o = 0,
          s;
        for (let n of t) {
          if ((ie(n, !0) && o++, o > 1)) break;
          ne(n, !0) && (s ??= n.id);
        }
        o === 1 && s && s !== r.id && e.push(s);
      }
      return (
        Be.debug("loadPartialDocument():", e),
        ge(this.parser, this.componentLoader, e, this.settings.treeServices)
      );
    }
    getScopesToLoad() {
...
var Y = T("app");
function Ke(h) {
  return h.treeReflectsDocument ? Fe(h.tree) : null;
}
function Fe(h) {
  return h.toJS();
}
...
function Qe(h, i) {
  ...
  try {
    ...
    Y.error(`Retrieving document \u201C${h}\u201D failed. (${r})`);
  }
  return G(e, i);
}
function Ce(h) {
...
function G(h, i) {
  ...
  try {
    ...
    throw (Ce(e), Y.warn("tree failed to verify:", t), t);
  }
}
...
var F = class extends pe.default {
  constructor(e, t, r, o) {
    super();
    ...
    ((this.scheduler = new Te(o.isUserIdleCallback)),
  ...
  prioritizeLoadingAllData(e) {
    ...
    let s = t || ("doNotTrack" in e && e.doNotTrack),
      ...
      d = this.tree ? de(this.tree) : void 0,
      l =
        ae.isOn("dataOnlyTree") && (d ? d === "crdt" : ue())
          ? this.afterAllDataLoaded(() => {
              if (n) return;
  ...
  async start() {
    return this.scheduler.run(async () => {
      ...
      if (
        ...
        !this.settings.partialParsing || (typeof t == "string" && !Se(t)))
      )
  ...
  parseFullDocumentSync(e) {
    ...
    let r = JSON.parse(e);
    if (!se(r.version)) throw Error("cannot read document version");
    if (
      ...
      return;
    let o = G(r, this.componentLoader);
...
function De(h, i) {
  if (i && (h.add(i.id), !!i.children)) for (let e of i.children) De(h, e);
}
var C = class {
    constructor(i, e) {
      c(this, "node", i);
...
`)
  ...
  Ne = class extends F {
    ...
    applySeqWatermarkToStore(e) {
      ...
      let t = ve().getSeqWatermark(this.projectId, e.branchId);
      if (t === void 0) return;
    ...
    switchToInitialBranch() {
      ...
      (this.branches.branchExists(t) ||
        ...
        this.projectId && Pe(this.projectId, t));
    }
    loadFirstCrdtTreeFromStore(e) {
      ...
      for (let u of s) {
        ...
        let Q = m.children.findIndex((ke) => ke.id === u);
        (Q >= 0 && m.children.splice(Q, 1, b), this.parsedIds.add(u));
        let Z = b;
        u === n?.maybeFirstPage.id &&
          n.firstDesignPage &&
          be(Z, n.firstDesignPage) !== Z &&
      ...
      return (
        M(l, v) &&
          (v.forEach((u) => {
            (t.push(`${u.id}: code component links itself via ${u.stack}`), q(l, u.id, u.stack));
          }),
    ...
    computeChunkingHints() {
      ...
      for (let a of n) t.getSubtreeCount(a) * s * Ee > 2e8 && e.add(a);
      return e;
    }
    ...
    async loadCrdtData(e = []) {
      ...
      for (let o = 0; o < E; ++o) {
        let s = await fetch(this.documentURL, r);
        if (s.ok) {
          await this.scheduler.yield();
        ...
        (s.status < 200 || s.status >= 300) &&
          (f.debug("onErrorStatusLoaded, retry:", o),
    ...
    resetForCrashRecovery(e) {
      ...
      return (
        M(d, m) &&
          (m.forEach((l) => {
            (t.push(`${l.id}: code component links itself via ${l.stack}`), q(d, l.id, l.stack));
          }),
...
export { xe as a, Pe as b, Ke as c, Xe as d, Qe as e, G as f, F as g, Re as h, Ne as i };
//# sourceMappingURL=chunk-ZAADFPOQ.mjs.map
